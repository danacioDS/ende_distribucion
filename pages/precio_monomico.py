...
def load_and_transform_data():
    try:
        ...
        price_columns = [col for col in df.columns if "Precio monómico US$/MWh" in col]
        ...
        for col in price_columns:
            ...
            temp_df['Precio monómico US$/MWh'] = temp_df[col]
...
with tab1:
    ...
    with col_left:
        ...
        precio_promedio_agente = df_agente['Precio monómico US$/MWh'].mean()

        fig_agente = px.line(
            df_agente,
            x='FECHA',
            y='Precio monómico US$/MWh',
    ...
    with col_right:
        ...
        df_empresa_prom = df_empresa.groupby(['FECHA', 'EMPRESA'])['Precio monómico US$/MWh'].mean().reset_index()
        precio_promedio_empresa = df_empresa['Precio monómico US$/MWh'].mean()

        if selected_empresa == "NO REGULADOS":
            df_no_regulados = df_filtered[df_filtered['EMPRESA'] == "NO REGULADOS"]
            precio_promedio_no_regulados = df_no_regulados['Precio monómico US$/MWh'].mean()
    ...
    df_sistema = df_filtered.groupby('FECHA')['Precio monómico US$/MWh'].mean().reset_index()
    df_sistema['Precio monómico US$/MWh'] = df_sistema['Precio monómico US$/MWh'].round(2)
    #df_sistema['FECHA'] = pd.to_datetime(df_sistema['FECHA'])
    precio_promedio_sistema = df_sistema['Precio monómico US$/MWh'].mean()
...
with tab2:
    ...
    df_empresas_prom_tab2 = df_filtered.groupby(['FECHA', 'EMPRESA'])['Precio monómico US$/MWh'].mean().reset_index()
    ...
    with col1:
        st.metric("Precio Mínimo Sistema", f"{df_filtered['Precio monómico US$/MWh'].min():.2f} US$/MWh")
    with col2:
        st.metric("Precio Promedio Sistema", f"{df_filtered['Precio monómico US$/MWh'].mean():.2f} US$/MWh")
    with col3:
        st.metric("Precio Máximo Sistema", f"{df_filtered['Precio monómico US$/MWh'].max():.2f} US$/MWh")
