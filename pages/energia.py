...
def load_and_transform_data():
    try:
        ...
        df = pd.read_excel(file_path, engine="openpyxl", 
                          usecols=lambda x: "Energía MWh" in x or x in ['AGENTE', 'EMPRESA'])
...
def plot_agent_energy(df_agente, agent_name):
    ...
    fig = px.line(
        ...
        x='FECHA',
        y='Energía MWh',
        title=f"Energía para {agent_name}",
        markers=True
    )
    fig.update_traces(
...
def plot_company_energy(df_empresa, company_name):
    ...
    # Agrupar por fecha
    df_grouped = df_empresa.groupby('FECHA', as_index=False)['Energía MWh'].sum()
    
    fig = px.line(
        df_grouped,
        x='FECHA',
        y='Energía MWh',
        title=f"Energía para {company_name}",
        markers=True
    )
    fig.update_traces(
...
with tab1:
    ...
    with col_left:
        ...
        if not df_agente.empty:
            ...
            energia_total_agente = df_agente['Energía MWh'].sum()
            energia_promedio_agente = df_agente['Energía MWh'].mean()
            porcentaje_agente = (energia_total_agente / total_energia_sistema) * 100
            
            col1, col2 = st.columns(2)
            col1.metric("Energía Promedio", f"{energia_promedio_agente:,.2f} MWh")
    ...
    with col_right:
        ...
        if not df_empresa.empty:
            ...
            # Métricas optimizadas
            energia_total_empresa = df_empresa['Energía MWh'].sum()
            energia_promedio_empresa = df_empresa.groupby('FECHA')['Energía MWh'].sum().mean()
            porcentaje_empresa = (energia_total_empresa / total_energia_sistema) * 100
            
            col1, col2 = st.columns(2)
            col1.metric("Energía Promedio", f"{energia_promedio_empresa:,.2f} MWh")
    ...
    if not df_filtered.empty:
        # Cálculo optimizado
        participacion = (
            df_filtered.groupby('EMPRESA', as_index=False)['Energía MWh']
            .sum()
...
with tab2:
    ...
    if not df_filtered.empty:
        ...
        fig_comparativo = px.line(
            ...
            markers=True,
            line_shape='spline',
            title="Comparación de Energía por Empresa"
        )
        ...
        # Tabla de resumen
        st.subheader("Resumen de Energía por Empresa")
        ...
        # Calcular energía por empresa por fecha
        empresa_por_mes = df_filtered.groupby(['FECHA', 'EMPRESA'], as_index=False)['Energía MWh'].sum()
