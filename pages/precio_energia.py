...
def load_and_transform_data():
    try:
        ...
        file_path = current_dir / "data" / "precios_con_empresas.xlsx"
        ...
        price_columns = [col for col in df.columns if "Precio Energía US$/MWh" in col]
        ...
        for col in price_columns:
            ...
            temp_df['Precio Energía US$/MWh'] = temp_df[col]
...
with tab1:
    ...
    with col_left:
        ...
        precio_promedio_agente = df_agente['Precio Energía US$/MWh'].mean()

        fig_agente = px.line(
            df_agente,
            x='FECHA',
            y='Precio Energía US$/MWh',
            title=f"Precios para {selected_agente}",
            markers=True,
            line_shape='linear'
        )
        fig_agente.update_traces(line=dict(width=3), marker=dict(size=8))
        fig_agente.update_layout(yaxis_title="Precio (US$/MWh)", xaxis_title="Fecha", showlegend=False)
    ...
    with col_right:
        ...
        df_empresa_prom = df_empresa.groupby(['FECHA', 'EMPRESA'])['Precio Energía US$/MWh'].mean().reset_index()
        precio_promedio_empresa = df_empresa['Precio Energía US$/MWh'].mean()

        if selected_empresa == "NO REGULADOS":
            df_no_regulados = df_filtered[df_filtered['EMPRESA'] == "NO REGULADOS"]
            precio_promedio_no_regulados = df_no_regulados['Precio Energía US$/MWh'].mean()
        ...
        fig_empresa.update_layout(yaxis_title="Precio Promedio (US$/MWh)", xaxis_title="Fecha", showlegend=False)
        st.plotly_chart(fig_empresa, use_container_width=True)

        cols_empresa = st.columns(2)
        with cols_empresa[0]:
            st.metric(label=f"Promedio {selected_empresa}", value=f"{precio_promedio_empresa:.2f} US$/MWh")


    # Evolución del Precio Promedio del Sistema (una sola línea)
    st.subheader("Evolución del Precio Promedio del Sistema")
    df_sistema = df_filtered.groupby('FECHA')['Precio Energía US$/MWh'].mean().reset_index()
    df_sistema['Precio Energía US$/MWh'] = df_sistema['Precio Energía US$/MWh'].round(2)
    #df_sistema['FECHA'] = pd.to_datetime(df_sistema['FECHA'])
    precio_promedio_sistema = df_sistema['Precio Energía US$/MWh'].mean()
    ...
    st.metric(label="Precio Promedio del Sistema", value=f"{precio_promedio_sistema:.2f} US$/MWh")

with tab2:
    st.header("Análisis Comparativo")
    st.subheader("Comparación de Empresas")

    df_empresas_prom_tab2 = df_filtered.groupby(['FECHA', 'EMPRESA'])['Precio Energía US$/MWh'].mean().reset_index()
    ...
    fig_comparacion.update_layout(
        yaxis_title="Precio Promedio (US$/MWh)",
        xaxis_title="Fecha",
    ...
    with col1:
        st.metric("Precio Mínimo Sistema", f"{df_filtered['Precio Energía US$/MWh'].min():.2f} US$/MWh")
    with col2:
        st.metric("Precio Promedio Sistema", f"{df_filtered['Precio Energía US$/MWh'].mean():.2f} US$/MWh")
    with col3:
        st.metric("Precio Máximo Sistema", f"{df_filtered['Precio Energía US$/MWh'].max():.2f} US$/MWh")
